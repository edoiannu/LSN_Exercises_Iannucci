...
public:
  ...
  double Cauchy_Lorentz(double mean, double gamma);
  // Method to randomly generate 1 or 0 (success of failure)
  bool Succ_fail();
  // Method to randomly generate a generic theta valure between 0 and pi according to the frequency function p(x) = 0.5 * sin(x)
  double Theta();

  void SetPrimes(int p1, int p2);

  double Distributionf(double (*fun)(double));
};
