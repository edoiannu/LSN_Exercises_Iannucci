...
public: // Function declarations
  ...
  void initialize(string inputf, string outputd); // Initialize system properties
  void initialize_properties(string outputd); // Initialize properties for measurement
  void finalize(string outputd);            // Finalize system and clean up
  void write_configuration(string outputd); // Write final system configuration to XYZ file
  void write_XYZ(int nconf);  // Write system configuration in XYZ format on the fly
  void write_velocities(string outputd);    // Write final particle velocities to file
  void print_exercise6(string outputd, int blk);
  void print_istant(string output);
  void read_configuration(string outputd);  // Read system configuration from file
  void initialize_velocities(string inputd);// Initialize particle velocities
  void step();                // Perform a simulation step
  void block_reset(int blk, string outputd);  // Reset block averages
  void measure();             // Measure properties of the system
  void averages(int blk, string outputd);     // Compute averages of properties
