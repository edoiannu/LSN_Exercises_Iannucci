...
void System :: Verlet(){
  ...
  _naccepted += _npart; // Number of accepted moves
  return;
}
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    delta_E = 2.0 * ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
      p = 1. / (1 + exp(-_beta*delta_E));
      if ( _rnd.Rannyu() < p ) { _particle(i).setspin(1); }
...
void System :: initialize(string inputf, string outputd){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory

  ifstream input(inputf); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open(outputd+"output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
...
void System :: initialize_velocities(string inputd){
  if(_restart and _sim_type==0){  // Se la variabile _restart è vera (cioè non zero) e il tipo di simulazione _sim_type è uguale a zero, allora la condizione è vera
    ...
    for (int i=0; i<_npart; i++){
      ...
      // cout << vx(i)*scalef << " " << vy(i)*scalef << " " << vz(i)*scalef << endl;
    }
  }
...
void System :: initialize_properties(string outputd){ // Initialize data members used for measurement of properties

  string property;
  int index_property = 0;
  _nprop = 0; // Number of properties being measured
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(outputd+"potential_energy.dat");
        ...
        ofstream coutt(outputd+"total_energy.dat");
        ...
        ofstream coutte(outputd+"temperature.dat");
        ...
        ofstream coutpr(outputd+"pressure.dat");
        ...
        ofstream coutpr(outputd+"magnetization.dat");
        ...
        ofstream coutpr(outputd+"cv.dat");
        ...
        ofstream coutpr(outputd+"chi.dat");
        ...
        ofstream coutf;
        coutf.open(outputd+"output.dat",ios::app);
  ...
  ofstream coutpr(outputd+"acceptance.dat");
  coutpr << "#     BLOCK:   ACCEPTANCE:" << endl;
  coutpr.close();
...
void System :: finalize(string outputd){
  this->write_configuration(outputd);
  _rnd.SaveSeed(/*outputd*/);
  ofstream coutf;
  coutf.open(outputd+"output.dat",ios::app);
...
void System :: write_configuration(string outputd){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(outputd+"CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    this->write_velocities(outputd);
  } else {
    coutf.open("INPUT/CONFIG/config.spin");
...
void System :: write_velocities(string outputd){
  ofstream coutf;
  coutf.open(outputd+"CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(string outputd){
  ifstream cinf;
  if(_restart){
    if(_sim_type > 1){
      int spin;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & term_config.xyz not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
    ...
    else { cinf.open("INPUT/CONFIG/config.ising"); }
  ...
  // else { cinf.open(outputd+"CONFIG/term_config.spin"); }

  return;
}

void System :: block_reset(int blk, string outputd){ // Reset block accumulators to zero
  ofstream coutf;
  if(blk>0){
    coutf.open(outputd+"output.dat",ios::app);
...
void System :: measure(){ // Measure properties
  _measurement.zeros();   // initialize to zero all the measured values of properties
  ...
  double temperature_temp=0.0; // temporary accumulator for temperature
  ...
  if (_measure_kenergy){
    ...
    // if (i==0) cout << dot( _particle(i).getvelocity() , _particle(i).getvelocity() ) << endl;}
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        ...
        tenergy_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j); // H
      }
      tenergy_temp /= double(_npart); // <H>/N
  ...
  if (_measure_temp and _measure_kenergy) {
    ...
    temperature_temp = (2.0/3.0) * kenergy_temp;
    _measurement(_index_temp) = temperature_temp;
  }
  // PRESSURE //////////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 4
  if (_measure_pressure) {
    pressure_temp /= _npart;
    _measurement(_index_pressure) = _ptail + _rho * temperature_temp + (16.0 * _rho * pressure_temp);
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if (_measure_magnet) {
    ...
    _measurement(_index_magnet) = magnetization / _npart;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if (_measure_cv) {
    cv_temp = _beta*_beta * _measurement(_index_tenergy)*_measurement(_index_tenergy);
    _measurement(_index_cv) = cv_temp * _npart; // qui sto già facendo la normalizzazione per il numero di particelle
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if (_measure_chi) {
    ...
    _measurement(_index_chi) = _beta * chi_temp * chi_temp / _npart;
  }
...
void System :: averages(int blk, string outputd){

  ofstream coutf;
  double average, sum_average, sum_ave2;

  _average     = _block_av / double(_nsteps); // mean of the current block
  if (_measure_cv) { _average(_index_cv) -=  _beta*_beta * _average(_index_tenergy)*_average(_index_tenergy)*_npart; }
  _global_av  += _average;                    // total mean until the current block
  _global_av2 += _average % _average; // % -> element-wise multiplication

  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(outputd+"potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(outputd+"kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(outputd+"total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(outputd+"temperature.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 4
  if (_measure_pressure){
    coutf.open(outputd+"pressure.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_magnet){
    coutf.open(outputd+"magnetization.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_cv){
    coutf.open(outputd+"cv.dat",ios::app);
    average = _average(_index_cv); // -  _beta*_beta * _average(_index_tenergy)*_average(_index_tenergy)*_npart;
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_chi){
    coutf.open(outputd+"chi.dat",ios::app);
...
void System :: print_exercise6(string outputd, int blk){

  ofstream coutre;
  if (_measure_tenergy){
    coutre.open(outputd+"total_energy.out",ios::app);
    coutre << _temp << setw(15) << _global_av(_index_tenergy)/double(blk) << setw(15) << this->error(_global_av(_index_tenergy), _global_av2(_index_tenergy), blk) << endl;
    coutre.close();
  }
  if (_measure_cv){
    coutre.open(outputd+"heat_capacity.out",ios::app);
    coutre << _temp << setw(15) << _global_av(_index_cv)/double(blk) << setw(15) << this->error(_global_av(_index_cv), _global_av2(_index_cv), blk) << endl;
    coutre.close();
  }
  if (_measure_chi){
    coutre.open(outputd+"susceptibility.out",ios::app);
    coutre << _temp << setw(15) << _global_av(_index_chi)/double(blk) << setw(15) << this->error(_global_av(_index_chi), _global_av2(_index_chi), blk) << endl;
    coutre.close();
  }
  if (_measure_magnet){
    coutre.open(outputd+"magnetization.out",ios::app);
    coutre << _temp << setw(15) << _global_av(_index_magnet)/double(blk) << setw(15) << this->error(_global_av(_index_magnet), _global_av2(_index_magnet), blk) << endl;
    coutre.close();
  }
}

void System :: print_istant(string outputd){

  ofstream coutre;
  if (_measure_temp){
    coutre.open(outputd+"istant_temp.out",ios::app);
    coutre << _measurement(_index_temp) << endl;
    coutre.close();
  }
}
